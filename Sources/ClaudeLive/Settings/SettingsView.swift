...
struct SettingsView: View {
    ...
    private var surfaceSection: some View {
        Section("Superficie") {
            Picker("Modalità", selection: $settings.displayMode) {
                // The notch option only appears on a Mac that has one, so it
                // can't be selected into a mode that cannot render.
            ...
            if !NotchGeometry.isAvailable {
                Text("Questo Mac non ha un notch: la modalità notch non è disponibile.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
            ...
            if settings.displayMode == .notch {
                ...
                Text("Nel notch l'interfaccia è sempre nera, per allinearsi al ritaglio fisico dello schermo.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            } else {
    ...
    private var availableModes: [DisplayMode] {
        NotchGeometry.isAvailable ? DisplayMode.allCases : [.floating]
    }
